...
| **外骨骼** | ESP32-S3 + 6×GT 模拟磁角度传感器 | 六轴角度采集与校准，经 P4 控制终端控制机器人 |
...
│   ├── s3-exoskeleton-sensor/      # ESP32-S3 六路 GT 模拟角度传感器
...
P4 对网页、外骨骼、Jog 与自检的所有关节目标统一执行 16ms 小步 ServoJ 插补。J1-J6 指令速度统一限制为 20°/s，单周期最大约 0.32°；六轴加速度统一限制为 10°/s²，并使用加加速度限制平滑启停。外骨骼目标按每个新传感器包更新，不再等待 200ms 或累计 1°。

外骨骼关节映射为 `H1→J1`、`H2→J2`、`H3→J4`、`H4→J3`、`H5→J6`、`H6→J5`。网页可为 H1-H6 独立设置角度增加的正/反方向，参数由 P4 保存到 NVS；修改方向前必须关闭外骨骼跟随。
...
| **v3.0** | 外骨骼六路 GT 磁角度采集 + WiFi/P4/Web 链路 | 🔜 进行中 |
