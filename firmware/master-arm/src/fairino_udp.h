...
public:
    ...
    bool     isRobotEnabled() const { return _robotEnabled; }
    ...
    // XML-RPC initialization (must be called before ServoJ!)
    int      robotEnable(uint8_t state);  // 0=disable, 1=enable
    int      mode(int m);                 // 0=Auto, 1=Manual
    ...
    int      sendXmlRpc(const char* method, const char* paramXml); // XML-RPC helper
    WiFiUDP  _udp;
    String   _ip;
