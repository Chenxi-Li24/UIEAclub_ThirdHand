...
| `exo direction <1-6> <-1\|1>` | 设置 H1-H6 的角度增加方向并保存到 NVS（须先关闭跟随） |
| `selftest` | 启动自检流程 |
| `estop` | 急停 |
| `help` | 命令帮助 |
...
所有运动目标先由 `SafeServoMotion` 从 CNDE 实际关节角开始插补，再由独立高优先级 FreeRTOS 网络任务按官方 UDP 示例连续下发。`cmdT=0.016s`，J1-J6 指令速度统一限制为 `20°/s`，单周期最大约 `0.32°`；六轴加速度统一限制为 `10°/s²`，软件同时限制加加速度。外骨骼目标按每个新序列包更新，不再使用 200ms 节拍和 1° 目标死区。SDK 中未开放的 `acc`、`vel`、`filterT`、`gain` 均固定为 `0`。
...
外骨骼与机械臂固定映射为 `H1→J1`、`H2→J2`、`H3→J4`、`H4→J3`、`H5→J6`、`H6→J5`。每个 H 通道可独立选择正向（`+1`）或反向（`-1`）；方向参数保存在 P4 NVS，不修改传感器原始角度或机械臂零点。
